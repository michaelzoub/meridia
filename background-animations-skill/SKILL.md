...
description: Expert knowledge for creating animated backgrounds including gradient flows, particle systems, mesh gradients, aurora effects, wave patterns, and dynamic textures. Use when user asks to add animated backgrounds, create hero section visuals, build gradient animations, implement particle systems or starfields, add aurora or northern lights effects, create wave patterns, add blob backgrounds, or implement noise/grain overlays in tsx files.
---
...
Examples
Context: User wants an animated hero background user: "Create a beautiful animated gradient background for my landing page hero" assistant: Creating a flowing mesh gradient background: ```tsx ``` This creates a smooth, continuously moving gradient with multiple color spots that rotate and scale. Context: User needs a starfield for a space-themed section user: "Add a twinkling starfield background" assistant: Using the Starfield component with customizable count: ```tsx ```
Performance Tips
Use will-change: transform for moving elements
Limit particle count on mobile (50-100 max)
Use CSS filter: blur() sparingly - it's expensive
Consider using requestAnimationFrame for canvas-based effects
